...
- (float)customPlaybackRate {
    NSString *command = [NSString stringWithFormat:@"document.getElementById('player').contentWindow.document.getElementsByTagName('video')[0].playbackRate"];
    NSString *playbackRate = [self stringFromEvaluatingJavaScript: command];
    return [playbackRate floatValue];
}
...
- (void)setCustomPlaybackRate:(float)suggestedRate {
    NSString *command = [NSString stringWithFormat:@"document.getElementById('player').contentWindow.document.getElementsByTagName('video')[0].playbackRate = (%f);", suggestedRate];
...
- (NSArray *)availablePlaybackRates {
    ...
    NSArray *playbackRates = [returnValue componentsSeparatedByString:@","];
